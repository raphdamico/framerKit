...
Drum = (parentDrumLayer, drumName, listItems, params) ->
	...
	listLayer.on Events.DragMove, =>
		if firstTouchAvailable
			@drumContainer.emit("DrumStartedMoving", {drum: drumName, index: @index, value: @val, velocity: 0})
	...
	listLayer.on Events.DragEnd, (e, f) =>
		...
		# Next touch should trigger DrumStartedMoving
...
exports.Picker = (params) ->
	...
	pickerStartedMoving = ()=>
		...
		newValues = for drum in @pickerContainer.drums
			drumValues[drum.name] = {index: drum.index, val: drum.val, velocity: 0}	
